...
export default function DashboardPage() {
  ...
  async function loadData() {
    ...
    const [
      ...
      supabase.from('food_logs').select('log_date, calories').eq('user_id', userId).gte('log_date', sevenDaysAgo).lte('log_date', today),
    ])
      ...
      try {
        const fbRes = await fetch('/api/ai', {
          ...
          body: JSON.stringify({
            ...
            data: { logs: foodLogs, targets: { calories: profile.daily_calorie_target, protein_g: profile.daily_protein_target }, date: today },
          }),
        })
        const { data: fbData } = await fbRes.json()
        if (fbData?.feedback) setFeedback(fbData.feedback)
      } catch (e) { console.error('AI feedback failed', e) }
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-green-700 text-sm mt-1 font-semibold">{format(new Date(), 'EEEE, MMMM d')}</p>
        </div>
      </div>
      ...
      {feedback && (
        <div className="bg-green-50 border border-green-200 rounded-2xl px-4 py-3 flex gap-3">
          ...
          <p className="text-sm text-green-900 leading-relaxed font-semibold">{feedback}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="card flex items-center gap-4">
          <div className="relative flex-shrink-0">
            <svg width="120" height="120" viewBox="0 0 120 120">
              <circle cx="60" cy="60" r={R} fill="none" stroke="#dcf2e6" strokeWidth="10" />
              <circle
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-lg font-black text-green-900">{totalCals}</span>
              <span className="text-xs text-green-600 font-bold">kcal</span>
            </div>
          </div>
          <div className="flex-1">
            <div className="text-sm font-bold text-green-900">Calories today</div>
            <div className="text-xs text-green-600 mt-1 font-semibold">Goal: {calTarget} kcal</div>
            <div className="text-xs text-green-500 mt-1 font-semibold">{calRemaining} remaining</div>
        ...
        <div className="card space-y-3">
          ...
          <MacroBar label="Protein" value={totalProtein} target={protTarget} unit="g" color="bg-green-500" />
          <MacroBar label="Carbs" value={foodLogs.reduce((s, l) => s + (l.carbs_g || 0), 0)} target={Math.round(calTarget * 0.45 / 4)} unit="g" color="bg-amber-400" />
          <MacroBar label="Fat"   value={foodLogs.reduce((s, l) => s + (l.fat_g || 0), 0)}   target={Math.round(calTarget * 0.3 / 9)}  unit="g" color="bg-emerald-400" />
        </div>

        {/* Workout summary */}
        <div className="card">
          <div className="text-sm font-bold text-green-900 mb-3">Today's activity</div>
          {workoutLogs.length === 0 ? (
            <div className="text-sm text-green-400 italic font-semibold">No workouts logged yet</div>
          ) : (
              ...
              {workoutLogs.map((w, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-green-600 font-semibold">{w.duration_minutes} min · {w.calories_burned || '—'} kcal</div>
                  </div>
                </div>
              ))}
            </div>
          )}
          <a href="/log" className="mt-3 block text-xs text-green-600 hover:text-green-800 font-bold">+ Log workout</a>
        </div>
      </div>
      ...
      <div className="card">
        ...
        {weeklyData.every(d => d.calories === 0) ? (
          <p className="text-sm text-green-400 italic text-center py-6 font-semibold">No data logged this week yet.</p>
        ) : (
          ...
          <div className="text-center py-8 text-green-400">
            ...
            <p className="text-sm font-semibold">
              {profile
                ...
                : <><a href="/profile" className="text-green-600 hover:underline font-bold">Complete your profile</a> first.</>
              }
            </p>
          </div>
        )}
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              {(['breakfast', 'lunch', 'dinner', 'snack'] as const).map(meal => (
                <div key={meal} className={`rounded-2xl border p-3 ${MEAL_COLORS[meal]}`}>
                  <div className="text-lg mb-1">{MEAL_ICONS[meal]}</div>
                  <div className="text-xs font-black text-green-800 capitalize">{meal}</div>
                  <div className="text-sm font-bold text-green-900 mt-1 leading-tight">{plan.meals[meal]?.name}</div>
                  <div className="text-xs text-green-600 mt-1 font-semibold">{plan.meals[meal]?.calories} kcal</div>
                </div>
              ))}
            </div>
            {plan.workout && (
              <div className="bg-green-50 border border-green-200 rounded-2xl p-3 flex items-start gap-3 mb-3">
                ...
                <div>
                  <div className="text-sm font-black text-green-900">{plan.workout.name} · {plan.workout.duration_minutes} min</div>
                  <div className="text-xs text-green-700 mt-0.5 font-semibold">{plan.workout.description}</div>
                </div>
              </div>
            )}
            {plan.tip && (
            ...
            <p className="text-xs text-green-300 mt-3 text-right font-bold">Cached for today · regenerate to refresh</p>
          </>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <a href="/log" className="card hover:border-green-400 hover:bg-green-50 transition-all cursor-pointer text-center py-5 block">
          <div className="text-3xl mb-2">📋</div>
          <div className="text-sm font-bold text-green-900">Log today's meals</div>
          <div className="text-xs text-green-500 mt-1 font-semibold">{foodLogs.length} items logged</div>
        </a>
        <a href="/calendar" className="card hover:border-green-400 hover:bg-green-50 transition-all cursor-pointer text-center py-5 block">
          <div className="text-3xl mb-2">📅</div>
          <div className="text-sm font-bold text-green-900">View calendar</div>
          <div className="text-xs text-green-500 mt-1 font-semibold">See your progress</div>
        </a>
      </div>
  ...
  return (
    <div>
      <div className="flex justify-between text-xs mb-1">
        ...
        <span className="text-green-600 font-semibold">{Math.round(value)}{unit} / {target}{unit}</span>
      </div>
