...
Generate a daily plan for today.`;
    ...
    if (action === "daily_feedback") {
      const { logs, workoutLogs, targets, date } = data;
      const totalCals = logs.reduce(
        (s: number, l: { calories: number }) => s + l.calories,
        0,
      );
      const totalProtein = logs.reduce(
        (s: number, l: { protein_g?: number }) => s + (l.protein_g || 0),
        0,
      );
      const totalCalsBurned = (workoutLogs || []).reduce(
        (s: number, w: { calories_burned?: number }) =>
          s + (w.calories_burned || 0),
        0,
      );
      const workoutSummary =
        (workoutLogs || []).length > 0
          ? (
              ...
              .map(
                (w) =>
                  `${w.workout_type} for ${w.duration_minutes} min (${w.calories_burned || "?"} kcal burned)`,
              )
              .join(", ")
          : "none";
      const prompt = `Date: ${date}
Calorie target: ${targets.calories} kcal, eaten: ${totalCals} kcal
Protein target: ${targets.protein_g}g, eaten: ${totalProtein}g
Meals logged: ${logs.map((l: { meal_type: string; food_name: string; calories: number }) => `${l.meal_type}: ${l.food_name} (${l.calories} kcal)`).join(", ") || "none"}
Workouts today: ${workoutSummary}${totalCalsBurned > 0 ? ` (${totalCalsBurned} kcal burned total)` : ""}
Write brief feedback covering both nutrition and activity.`;
